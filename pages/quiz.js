          ...
          <div className="bg-[#0c0c0c] p-8 rounded-3xl border border-slate-800">
            ...
            <div className="grid gap-3">
              {questions[current].options.map((opt, i) => (
                <button key={i} disabled={answered} onClick={() => { setAnswered(true); setSelectedOption(i); if(i === questions[current].answer) setScore(s => s + 1); setAttempts([...attempts, { question: questions[current].question, options: questions[current].options, selected: i, correct: questions[current].answer }]); }} className={`p-4 rounded-xl border text-left ${answered && i === questions[current].answer ? 'bg-green-600/20 border-green-500' : answered && i === selectedOption ? 'bg-red-600/20 border-red-500' : 'bg-[#151515] border-slate-800'}`}>{opt}</button>
              ))}
            </div>
            {answered && <button className="w-full mt-6 bg-yellow-600 py-3 rounded-xl font-bold" onClick={() => { if(current + 1 < questions.length) { setCurrent(current + 1); setAnswered(false); setSelectedOption(null); } else { setQuizState('finished'); } }}>{current + 1 < questions.length ? 'Next Question' : 'Finish Quiz'}</button>}
          </div>
        )}
        {quizState === 'finished' && (
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    <p className="text-xl font-bold my-4">Score: {score} / {questions.length}</p>
                    ...
                    <button onClick={async () => { if(score >= 7 && !unlockedLevels.includes(selection.level + 1)) { const newL = [...unlockedLevels, selection.level + 1]; setUnlockedLevels(newL); localStorage.setItem('unlockedLevels', JSON.stringify(newL)); } await fetch('/api/leaderboard', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ name, score, total: questions.length }) }); window.location.href = '/leaderboard'; }} className="w-full bg-yellow-600 py-4 rounded-xl font-bold mb-4">SAVE SCORE</button>
